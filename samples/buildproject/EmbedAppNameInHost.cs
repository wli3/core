...
namespace Cli.Prototype.Utils
{
    ...
    public static class EmbedAppNameInHost
    {
            ...
            string appBinaryFilePath)
        {
            ...
            {
                throw new EmbedAppNameInHostException(string.Format("Given file name '{0}' is longer than 1024 bytes", appBinaryFilePath));
            }
        ...
        private static void SearchAndReplace(byte[] array, byte[] searchPattern, byte[] patternToReplace, string appHostSourcePath)
        {
            ...
            if (offset < 0)
            {
                throw new EmbedAppNameInHostException(string.Format("Unable to use '{0}' as application host executable as it does not contain the expected placeholder byte sequence '{1}' that would mark where the application name would be written.", appHostSourcePath, _placeHolder));
            }
